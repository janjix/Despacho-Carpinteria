...
## Escanear desde el televisor

Funciona: el panel vive dentro del mismo proveedor de escaneo que el resto de
la app, así que un lector conectado al TV Box registra igual que uno conectado
a la laptop.

**El camión no se elige en ninguna pantalla.** En el taller se carga uno a la
vez, así que el activo es sencillamente el viaje que esté en estado
`cargando`. Se abre desde Camiones en cualquier dispositivo y todas las
pantallas escanean contra él.

Antes había un selector que guardaba el viaje en cada navegador, y eso rompía
justo este flujo: se abría el viaje en la laptop, el televisor no se enteraba,
y todo lo escaneado desde ahí quedaba sin camión. Ahora se deduce del estado
de la base y no hay nada que configurar.

Si por error quedaran dos viajes abiertos, se usa el más reciente y las
pantallas lo advierten. Si no hay ninguno, una franja amarilla lo avisa en
grande: lo que se embale se registra igual, pero lo que se cargue quedará sin
asignar.

Cada escaneo muestra una franja a todo lo ancho con el resultado en el color
del estado, el código y la hora, durante seis segundos. Los rechazos se ven
aunque el bulto no esté en pantalla.

Dicho esto, el operario que embala está mejor con la app cerca: a tres metros
se ve el color y el salto de página, pero no el detalle de lo que se leyó.

## El navegador del televisor

Funciona directamente en el navegador de un Samsung, sin TV Box, y el lector
USB conectado al televisor también funciona: Tizen lo trata como teclado y la
app escucha igual que en una laptop.

Dos cosas hay que saber:

**No trae `ResizeObserver`.** Por eso la medición del espacio no depende de
él: hace varios reintentos al arrancar (0, 60, 200, 600, 1500 y 3000 ms) y
después mantiene un latido lento que solo actualiza si el tamaño cambió de
verdad. Sin eso, en un televisor se medía una única vez con el layout a medio
resolver, salía capacidad para una fila y el panel quedaba casi vacío con
decenas de páginas.

**Puede reportar la ventana como 0x0.** Por eso el panel se ancla con
`position: fixed` e `inset: 0` en lugar de depender de `100vh`, que en ese
caso valdría cero y colapsaría toda la cadena de alturas. Y la medición de
cada zona prueba cuatro fuentes en orden: `clientHeight`,
`getBoundingClientRect`, `offsetHeight`, y por último el hueco entre donde
empieza el contenedor y el fondo de la ventana. Si aun así no hay medida, el
grid se dibuja en flujo normal: es preferible que desborde un poco a que la
pantalla quede vacía.

**La barra de direcciones se lleva alto.** En *Ajustes → Esta pantalla* se ve
lo que la app percibe: resolución, densidad, zoom del navegador y cuántos
píxeles se está comiendo la barra. Si son muchos, aparece el botón *Recuperar
ese espacio*, que entra en pantalla completa. En un televisor no hay consola,
así que ese recuadro es la única forma de diagnosticar sin adivinar.

## Navegadores antiguos

Los TV Box baratos traen un Chrome viejo que no se actualiza solo. Ahí
sintaxis moderna como `?.` o `??` no existe, el bundle revienta en la primera
línea y **la pantalla queda en blanco sin ningún mensaje**, que es lo peor que
puede pasar en un equipo sin teclado.

Dos medidas contra eso:

- `@vitejs/plugin-legacy` genera un segundo bundle transpilado con sus
  polyfills, dirigido a Chrome 64 y Android 6. El navegador moderno descarga
  el normal e ignora el otro; el viejo hace lo contrario. Pesa más en disco,
  pero nadie descarga los dos.
- `index.html` lleva una red de seguridad: si el JavaScript lanza un error, o
  si a los ocho segundos React no ha dibujado nada, aparece una pantalla oscura
  con el detalle técnico y el user agent del equipo. En un televisor eso es la
  diferencia entre saber qué pasó y adivinar.

Para comprobar la versión de Chrome del TV Box: abre `chrome://version`. Por
debajo de 64 no hay nada que hacer desde la app; hay que actualizar el
navegador o usar otro equipo.
...
- **Orden alfabético dentro de cada área.** Buscar un nombre en una pared es
  más rápido que buscarlo en el orden en que se cargó la lista.
- **Pantalla dividida por proyecto.** En *Ajustes → Proyecto* está la opción
  *Todos los proyectos activos*: la pantalla se reparte en columnas, una por
  obra, cada una con su cabecera, sus contadores y **su propia paginación**.

  Mezclar las tarjetas de dos obras en una sola lista obliga a leer el nombre
  de cada una para saber cuál es cuál. Con la pantalla dividida, cada obra
  tiene su sitio fijo y el ojo va directo.

  Cada zona mide su propio espacio y calcula cuántas tarjetas caben ahí dentro:
  una columna estrecha admite menos que la pantalla entera, y el número tiene
  que salir de la medida real. Las zonas rotan independientes, así que un
  escaneo salta a su página sin mover las demás.

  En vertical las zonas se apilan en filas en lugar de columnas.
- **La cabecera y el pie ocupan lo mínimo.** Lo que importa son las tarjetas:
  en 1080p a tamaño medio caben 49 en lugar de 30.
...
- **Sin scroll, y sin filas cortadas.** El panel mide el espacio real de la
  rejilla con un `ResizeObserver` y calcula cuántas tarjetas caben: columnas
  por filas enteras. Lo que no entra pasa a la página siguiente, que rota sola
  cada doce segundos.

  Antes el número era fijo y elegido a mano, y por eso la última fila se
  cortaba: cuántas entran depende del tamaño de letra, la orientación, cuántos
  proyectos hay en pantalla y el televisor concreto. Ese ajuste ya no existe
  porque nunca podía acertar.

  **Aprovechar el sitio.** La tarjeta mide lo que necesita: padding, nombre a
  dos líneas, área y pie. Antes reservaba tres líneas de nombre que casi nunca
  se usan y desperdiciaba 37 px por tarjeta. Los nombres largos bajan de cuerpo
  en lugar de robar altura, así el alto es fijo y el cálculo de filas exacto.

  Además, la escala se ajusta hasta un 8% hacia abajo si con eso entra otra
  fila entera. El margen es estrecho a propósito: sirve para no dejar noventa
  píxeles en blanco por veinte que faltan, no para llenar la pantalla de letra
  menuda. En empate gana siempre la escala mayor.

  El resultado, medido: en 1080p a tamaño medio se pasa de 49 tarjetas a 81, y
  con dos proyectos en pantalla de 18 por zona a 32, sin bajar de escala 0.93.

  **El grid va en capa absoluta.** El contenido de cada zona se dibuja con
  `absolute inset-0` dentro del contenedor que se mide. Si el grid pudiera
  empujar ese contenedor, cada medición cambiaría su propio resultado: mide,
  calcula menos tarjetas, el contenido encoge, mide menos. Bastaba con estar
  en una página incompleta para que colapsara a una fila y decenas de páginas,
  y cada escaneo lo empeoraba.

  **La cadena de alturas.** Para que la medida sea el espacio libre y no el
  contenido, cada eslabón del árbol necesita altura definida: la rejilla de
  zonas con `h-full`, el envoltorio de cada zona con `h-full`, la sección con
  `h-full`, y solo entonces el contenedor que se mide con `flex-1 min-h-0`. Si
  falta uno, el `flex-1` no tiene de dónde crecer, mide su propio contenido y
  devuelve capacidad para una tarjeta: decenas de páginas con la pantalla medio
  vacía. Es el fallo más fácil de introducir y el más difícil de ver.

  En *Ajustes → Densidad* está **Cómoda**, que devuelve la tarjeta alta si
  prefieres nombres más grandes a costa de ver menos.

  Para que el cálculo sea exacto, la tarjeta tiene altura fija y el nombre se
  recorta a tres líneas. Si creciera con el texto, el número de filas dejaría
  de ser predecible.

  Si con el tamaño elegido no entra ni una fila completa (pantalla pequeña, o
  densidad mal configurada en el TV Box), la letra se reduce sola hasta que
  quepa. Letra algo menor es mejor que una tarjeta cortada. Nadie va a bajar con la rueda del ratón
...
- **La tira se llena a medida que se escanea.** Cada barrita toma el color de
  su bulto: amarillo al embalar, verde al cargar. Como se dibuja sobre dos
  fondos distintos, el amarillo se oscurece cuando la tarjeta ya es amarilla,
  que si no sería invisible.

  La tira sigue visible mientras quede algo por cargar. Antes desaparecía en
  cuanto el mueble estaba embalado del todo, justo cuando empieza a importar
  cuántos bultos han subido ya al camión.

  Con más de catorce bultos las barritas quedarían de dos píxeles, así que se
  sustituyen por una barra continua de dos tramos: dice lo mismo y se ve desde
  igual de lejos.

  El pie de la tarjeta cuenta las dos etapas (`6 embalados · 2 cargados`), no
  solo los cargados como antes.

  **El pie tiene sitio fijo y el nombre usa lo que sobra.** Antes era al
  revés: el nombre tenía altura propia y con dos líneas empujaba fuera el pie
  y la tira, que quedaban recortados por el overflow sin avisar. Medido, se
  salían hasta 27 px, así que en los muebles de nombre largo la tira
  simplemente no existía. La altura de la tarjeta (7.4rem) está calculada para
  que quepan dos líneas de nombre en los tres cuerpos, y no cuesta ninguna
  tarjeta en 1080p porque el reparto de filas cae igual que con 6.6.
- **El panel salta a la página del bulto recién escaneado** y lo destaca con
  un borde blanco durante seis segundos. En una pantalla que rota sola, ver el
  cambio es lo que confirma al operario que el escaneo entró: si el mueble
  está en la página cuatro, no se entera de nada. El temporizador de doce
  segundos se reinicia con el salto, así que la página que acaba de aparecer
  no cambia un segundo después.
...
esquina. Ocupa todo el alto y su contenido tiene scroll propio, porque en un
televisor no hay rueda de ratón: con el control remoto las flechas mueven el
foco entre botones y el scroll sigue al foco. Antes eran una barra superior que tapaba la cabecera justo cuando
...
2. SQL Editor → ejecutar en este orden, cada archivo completo y sin nada
   ...
   | `supabase/parche-05-estado-manual.sql` | siempre |
   | `supabase/parche-06-viaje-activo.sql` | siempre, al final |
   ...
   **Para saber qué falta**, ejecuta `supabase/verificar-instalacion.sql`. No
   cambia nada: lista las tablas, funciones, vistas y triggers que no están.
   Si la app dice *Falta una parte de la base de datos*, empieza por ahí.
...
Estados de un viaje: `cargando` → `despachado` → `entregado`. **Solo
`cargando` admite carga.** Un viaje despachado ya salió del taller, y
registrar que un bulto subió a un camión que no está es exactamente lo
contrario de lo que esta app existe para evitar.

Un bulto escaneado cuando el camión ya salió se marca como cargado igual, pero
queda sin asignar y aparece en la lista de huérfanos para resolverlo a mano.
Asignar manualmente a un viaje despachado también se rechaza.
...
Pestaña **Herrajes**: catálogo común a todos los proyectos, con código propio,
marca, medida y unidad. Se carga una vez y se reutiliza.

Dentro de un proyecto, cada área tiene el botón **Bulto de herrajes**. Se elige
qué va dentro y sale **un solo ítem con una sola etiqueta**.

Eso último es la decisión de fondo. El contenido no son ítems: una caja con
veinticuatro bisagras y seis correderas es UN bulto que sube al camión una
vez. Si cada herraje fuera un ítem habría que escanear treinta etiquetas y los
conteos del panel dirían cualquier cosa.

Un bulto de herrajes se escanea, se embala y se carga exactamente igual que un
mueble, sin ningún caso especial.

- **La cantidad es opcional.** Hay cosas que se cuentan (24 bisagras) y cosas
  que no (una bolsa de tornillos surtidos). Cada línea admite además una nota.
- **Varios bultos por área**, tantos como haga falta.
- **La etiqueta** lleva una marca negra que dice HERRAJES y cuántas piezas
  distintas van dentro. La lista completa no cabe ahí y tampoco hace falta:
  vive en la guía, que es donde se comprueba.
- **La guía de despacho** lista debajo de cada bulto todo lo que contiene, con
  cantidades y notas. La casilla de verificación es solo del bulto: en el
  portón se cuentan bultos, no bisagras.

Un herraje que ya esté dentro de algún bulto no se puede borrar del catálogo;
la app lo dice en lugar de dejar registros huérfanos.

## Lo despachado sale de la vista

Cuando un viaje pasa a *despachado* o *entregado*, sus bultos se apartan en
dos sitios a la vez:

- **En el panel** desaparecen, como se explica arriba.
- **En el proyecto** bajan a una sección plegada al final, *Ya despachados*,
  agrupada por viaje: camión, código, destino, hora de salida y el detalle de
  qué se fue.

No se borra nada. Siguen en la guía de su viaje, en la lista de embalaje y en
la de carga, que se generan con el proyecto completo. Lo que cambia es que
dejan de mezclarse con lo que queda por hacer: en un proyecto de 150 bultos,
ver los 60 que ya se fueron entorpece más de lo que informa.

## La marca: FATE

La app se llama FATE Despacho y lleva el logo en la barra de navegación, en el
pie del panel del televisor, en las etiquetas y en las listas PDF. La guía de
despacho no lo lleva a propósito: se imprime sobre papel con membrete.

En la etiqueta va el **isotipo** arriba a la izquierda. Su hueco se reserva
siempre y el encabezado (proyecto y área) se centra en el espacio que queda a
la derecha: si no cabe en una línea a 12 pt se prueba a 11, y si tampoco, se
parte en dos. La etiqueta no lleva el nombre del cliente.

Los archivos del logo están en `public/`: `fate-negro.png` para fondos claros,
`fate-blanco.png` para los oscuros y `favicon.png` con el isotipo. Para los
PDF va incrustado en `src/lib/logo.js`, porque jsPDF necesita los bytes al
dibujar y pedirlos a la red desde el taller es un fallo gratuito.

**Los códigos nuevos empiezan por FATE.** El parche 08 cambia el prefijo, que
ahora sale de una sola función (`prefijo_codigo()`): si la marca vuelve a
cambiar, se edita esa línea y nada más.

Las etiquetas TECC ya pegadas siguen funcionando. El escaneo busca el código
exacto de cada bulto y un código nunca cambia después de creado, así que en un
mismo proyecto conviven bultos TECC-… viejos y FATE-… nuevos. La numeración
continúa: si el área iba por el 058, el siguiente es FATE-…-059. Reimprimir
una etiqueta vieja la saca con su TECC.

## Tipografía

La app usa **Rubik** en toda la interfaz, y IBM Plex Mono solo para los
códigos. Rubik se sirve desde `public/fonts/` en lugar de Google Fonts: cinco
pesos (400 a 800) en woff2, recortados al alfabeto latino con acentos y ñ, unos
95 KB en total. Así el televisor no depende de un servicio externo para verse
bien. La licencia OFL va junto a los archivos.

**Cambiarla no alteró ninguna altura.** Todas las líneas de la app tienen el
interlineado fijado, así que miden lo mismo con cualquier fuente; lo que cambia
es el ancho. Medido con las fuentes reales: las mayúsculas (áreas, pie de las
tarjetas) varían menos de un 2%, y los nombres en negrita salen un 5 a 7% más
anchos. Con diecisiete nombres reales y varios anchos de tarjeta, ninguno pasa
de dos líneas a tres; los pocos que cambian van de una a dos, y la tarjeta
tiene sitio para ambas. Por eso los tamaños se dejaron como estaban.

Las etiquetas y los PDF siguen en Helvetica, que es la fuente que jsPDF lleva
incorporada; meter Rubik ahí añadiría peso a cada documento sin ganar nada en
el papel.

## Áreas y colores

En Proyectos las áreas están **plegadas**, y cada una dice cuánto falta:
*Faltan 9 · 6 por embalar · 3 por cargar · de 12*. Falta es todo lo que no ha
subido todavía a un camión. Un área terminada dice *Completa*. La barra de
avance se ve aunque esté plegada. Lo que cada uno despliega se recuerda por
proyecto, y hay botones para desplegar o plegar todas.

Cada área tiene un **color**, el mismo aquí y en el televisor: en el panel el
nombre del área va subrayado con su color, y en Proyectos lleva un cuadrito al
lado. El subrayado es un borde inferior y no `text-decoration`, porque el
navegador de los Samsung no entiende el grosor ni el color de este último.
Quien ve un mueble en la pantalla y luego lo busca en la laptop reconoce el
área por el color. La paleta (azul, violeta, cian, fucsia, lavanda y rosa) está
elegida para no confundirse con el gris, el amarillo, el verde ni el rojo, que
ya significan algo. Se asigna por el orden del área dentro de su proyecto; con
más de seis áreas se repite, y ahí desempata el nombre.

El panel muestra una leyenda con las áreas de la página a la vista y su color.

## Medir el proceso

`supabase/metricas.sql` trae ocho consultas de solo lectura: resumen por
proyecto, ritmo de trabajo en bultos por hora, calidad de la lectura,
correcciones manuales por semana, bultos sin camión, espera entre embalar y
cargar, completitud de cada viaje, e incidencias en obra.

Todo sale de la tabla `escaneos`, que registra cada lectura con su hora, su
acción y su resultado. No hubo que instrumentar nada: es el rastro que la app
deja al trabajar.

**Lo que estas métricas no pueden hacer.** No demuestran la mejora frente al
método anterior, porque de aquello no quedó ningún dato. Miden cómo va el
proceso hoy y sirven de línea base hacia adelante.

El indicador que de verdad le importa al negocio (cuántas veces hubo que
volver a la obra por un bulto que no llegó) ocurre fuera de la app. Por eso el
bloque 8 crea la tabla `incidencias_obra`, que se llena a mano: fecha,
proyecto, tipo, bultos afectados, si hubo regreso y cuánto costó. Son treinta
segundos por incidencia y es el único dato que convierte las demás cifras en
un argumento.
...
Sale en un ejemplar. El botón **3 copias**, al lado, genera los tres marcados
*original taller*, *copia transporte* y *copia obra*: son la misma guía
repetida, no páginas de más.
...
### El viaje activo es del taller, no del navegador

Solo se carga un camión a la vez, así que el viaje activo vive en la base y
llega a todos los dispositivos por Realtime.

Eso resuelve el flujo real: el viaje se abre desde la laptop cuando empieza la
carga, y se escanea desde el televisor del galpón. Antes el activo se guardaba
en el `localStorage` de cada navegador, así que el televisor no se enteraba de
lo que se abría en la laptop y todo lo que se escaneaba allí quedaba cargado
sin camión asignado.

Reglas:

- **Abrir un viaje lo deja activo.** Se abre justo cuando se va a empezar a
  cargar, así que es lo que espera quien lo abre.
- **Marcarlo como despachado lo desactiva.** Un camión que salió no debe
  recibir escaneos por omisión; si no, el primer escaneo del día siguiente se
  cargaría en un camión que ya está en la obra. Sigue admitiendo carga si
  alguien lo elige a mano, para el bulto que se subió en el último momento.
- **Nunca hay dos activos.** Un índice único lo garantiza en la base, no en
  el código.
- **Se puede cambiar desde cualquier pantalla**: el cajón de ajustes del
  panel, el selector de la pantalla de escaneo, o el botón *Cargar en este
  camión* del detalle del viaje. Cambiarlo en un sitio lo cambia en todos.

El escaneo no manda el viaje desde el navegador: lo resuelve la base en el
momento de registrar. Así un dispositivo con datos viejos en pantalla no puede
cargar contra un camión que ya cambió.
