...
function tabla(autoTable, doc, contenido, y) {
  ...
  for (const [clave, lista] of grupos) {
    ...
    autoTable(doc, {
      ...
      didDrawCell: (datos) => {
        // Cuadrito de verificación dibujado a mano
 ...
 */
export async function guiaDespacho(viaje, contenido, copias = 1) {
  ...
  for (let c = 0; c < Math.max(1, copias); c++) {
    ...
    if (!contenido.length) {
      ...
      y = tabla(autoTable, doc, contenido, y)
    }
