...
import { ISOTIPO_PDF, ISOTIPO_PROPORCION } from './logo'

async function cargarLibrerias() {
  const [pdf, barras] = await Promise.all([import('jspdf'), import('jsbarcode')])
  // El constructor llega como default o como export con nombre según cómo
  // empaquete el entorno. Se elige el que de verdad sea una función.
  const elegir = (...c) => c.find(x => typeof x === 'function')
  return {
    jsPDF: elegir(pdf.default, pdf.jsPDF, pdf.default?.jsPDF),
    JsBarcode: elegir(barras.default, barras.default?.default, barras)
  }
}
...
function dibujarEtiqueta(doc, item, ctx) {
  ...
  // Franja superior: el isotipo a la izquierda y de dónde viene el bulto.
  //
  // El hueco del isotipo se reserva siempre y el encabezado se centra en el
  // espacio que queda a su derecha. Si se centrara en el ancho total, con
  // áreas largas como "Vestier y hab ppal" el texto llegaría hasta el borde y
  // no habría sitio para la marca. Si no cabe en una línea a 12 pt se prueba a
  // 11, y si tampoco, se parte en dos líneas antes que encogerlo más: es lo
  // primero que busca el operario entre veinte bultos apilados.
  const altoIso = 5.4
  const anchoIso = altoIso * ISOTIPO_PROPORCION

  const inicioTexto = MARGEN + anchoIso + 2
  const disponible = derecha - inicioTexto
  const centro = inicioTexto + disponible / 2
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(0)

  const cabecera = `${proyecto.nombre} · ${area.nombre}`.toUpperCase()
  let unaLinea = false
  for (const cuerpo of [12, 11]) {
    doc.setFontSize(cuerpo)
    if (doc.getTextWidth(cabecera) <= disponible) {
  ...
  if (!unaLinea) {
    // Dos líneas; cada una se encoge solo si no cabe a su cuerpo normal
    const ajustar = (texto, cuerpo, minimo) => {
      let c = cuerpo
      doc.setFontSize(c)
      while (c > minimo && doc.getTextWidth(texto) > disponible) { c -= 0.5; doc.setFontSize(c) }
    }
    ajustar(proyecto.nombre.toUpperCase(), 11, 8)
    doc.text(proyecto.nombre.toUpperCase(), centro, 5.6, { align: 'center' })
    ajustar(area.nombre.toUpperCase(), 10, 7.5)
    doc.text(area.nombre.toUpperCase(), centro, 9.4, { align: 'center' })
  }

  // Centrado en la altura de la franja, que es mayor con dos líneas
  doc.addImage(ISOTIPO_PDF, 'PNG', MARGEN, unaLinea ? 2.1 : 3.0,
    anchoIso, altoIso, undefined, 'FAST')

  const yLinea = unaLinea ? 9.5 : 11.5
