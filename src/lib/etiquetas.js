...
function dibujarEtiqueta(doc, item, ctx) {
  ...
  doc.setFontSize(8)
  doc.setTextColor(60)
  doc.text(`${proyecto.nombre} · ${area.nombre}`.toUpperCase(), MARGEN, 6.5)
  if (proyecto.cliente) {
    doc.text(proyecto.cliente.toUpperCase(), derecha, 6.5, { align: 'right' })
  }

  doc.setDrawColor(160)
  doc.setLineWidth(0.2)
  doc.line(MARGEN, 8.5, derecha, 8.5)
  ...
  doc.setFontSize(9)
  doc.setTextColor(40)
  const detalle = [item.medidas, item.material].filter(Boolean).join('  ·  ')
  if (detalle) doc.text(detalle, MARGEN, Math.max(y + 0.5, 27.5))

  // Contador de bultos, solo cuando el grupo tiene más de uno
  if (hayContador) {
    doc.setDrawColor(0)
    doc.setLineWidth(0.4)
    doc.rect(derecha - 20, 11, 20, 13)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(13)
    doc.setTextColor(0)
    doc.text(`${item.indice} / ${item.cantidad}`, derecha - 10, 19.5, { align: 'center' })
  }
