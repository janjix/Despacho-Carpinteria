...
export function parsearLista(texto, { cantidadAlInicio = true } = {}) {
  ...
  for (const cruda of (texto ?? '').split(/\r?\n/)) {
    ...
    // Un nombre repetido suma cantidad en lugar de duplicar el renglón
    const clave = limpio.toUpperCase()
    if (vistos.has(clave)) {
      vistos.get(clave).cantidad += cantidad
      continue
    }
    const fila = { nombre: limpio, cantidad, medidas: '', material: '', descripcion: '' }
...
export function resumenParseo(filas) {
  ...
  const repetidos = filas.filter(f => f.cantidad > 1).length
  return { renglones: filas.length, etiquetas, repetidos }
}
