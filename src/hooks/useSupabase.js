...
export function useProyectos() {
  ...
  const crear = useCallback(async ({ nombre, cliente, fecha_despacho }) => {
    ...
    if (error) throw new Error(traducirError(error, 'crear_proyecto'))
    await recargar()
...
export async function crearArea(proyectoId, nombre, orden = 0) {
  ...
  if (error) throw new Error(traducirError(error, 'crear_area'))
  return Array.isArray(data) ? data[0] : data
}
...
function traducirError(error, funcion) {
  // PGRST202: la función no existe en el esquema. Casi siempre significa que
  // falta ejecutar un parche SQL, y el mensaje crudo no lo dice.
  if (error.code === 'PGRST202' || /Could not find the function/i.test(error.message ?? '')) {
    return `Falta una parte de la base de datos${funcion ? ` (${funcion})` : ''}. ` +
           'Ejecuta los parches pendientes de la carpeta supabase en el SQL Editor, ' +
           'en orden numérico, y vuelve a intentar.'
  }
  if (error.code === '23505') {
    return 'Ese código ya existe. Vuelve a intentar; si se repite, avisa para ' +
           'revisar los códigos del proyecto.'
  }
  if (error.code === '23503') {
    return 'El área o el proyecto ya no existen. Recarga la página.'
  }
  if (error.code === '42501' || /permission denied/i.test(error.message ?? '')) {
    return 'La base de datos rechazó la operación por permisos. Revisa las ' +
           'políticas RLS de la tabla.'
  }
...
export async function registrarEscaneo(codigo, modo, viajeId = null) {
  ...
  if (error) throw new Error(traducirError(error, 'escanear'))
  return data
}
...
/**
 * Viajes que todavía admiten carga, y cuál es el activo del taller.
 * Realtime propaga el cambio: si en la laptop se abre otro viaje, el
 * televisor empieza a cargar contra ese sin que nadie lo toque allí.
 */
...
export async function asignarAViaje(viajeId, itemIds) {
  ...
  if (error) throw new Error(traducirError(error, 'asignar_a_viaje'))
  return data
}
...
export async function cambiarEstadoItem(itemId, estado, motivo, viajeId = null) {
  ...
  if (error) throw new Error(traducirError(error, 'cambiar_estado_item'))
  return data
}
...
// Datos del panel
//
// Puede mostrar un proyecto o todos los activos a la vez, para cuando se
// embalan dos obras en paralelo y hay un solo televisor.
// ---------------------------------------------------------------------

export function usePanelDatos(proyectoId) {
  const [proyectos, setProyectos] = useState([])
  const [areas, setAreas] = useState([])
  const [items, setItems] = useState([])
  const [cargando, setCargando] = useState(true)
  ...
  const todos = proyectoId === 'todos'

  const recargar = useCallback(async () => {
    // No se filtra por estado: la app nunca lo escribe ni lo deja cambiar,
    // así que un proyecto con cualquier otro valor desaparecía del panel sin
    // que nadie supiera por qué. Lo que decide es no estar borrado.
    let consulta = supabase.from('proyectos').select('*')
      .is('deleted_at', null)
      .neq('estado', 'archivado')
    if (!todos && proyectoId) consulta = consulta.eq('id', proyectoId)

    const { data: pr, error: eP } = await consulta
      .order('fecha_despacho', { ascending: true, nullsFirst: false })
    if (eP) { setError(eP.message); setCargando(false); return }

    const lista = pr ?? []
    setProyectos(lista)
    if (!lista.length) { setAreas([]); setItems([]); setCargando(false); return }

    const { data: ar } = await supabase.from('areas').select('*')
      .in('proyecto_id', lista.map(p => p.id))
      .is('deleted_at', null).order('orden')
    const listaAreas = ar ?? []
    setAreas(listaAreas)

    if (!listaAreas.length) { setItems([]); setCargando(false); return }

    const { data: it, error: eI } = await supabase.from('items').select('*')
      .in('area_id', listaAreas.map(a => a.id))
      .is('deleted_at', null)
    if (eI) setError(eI.message)
    else { setItems(it ?? []); setError(null) }
    setCargando(false)
  }, [proyectoId, todos])

  useEffect(() => { setCargando(true); recargar() }, [recargar])

  // Realtime sobre los ítems de las áreas que estamos mostrando
  useEffect(() => {
    if (!areas.length) return
    const idsArea = new Set(areas.map(a => a.id))
    const canal = supabase
      .channel(`panel-${proyectoId ?? 'todos'}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'items' }, (payload) => {
        const fila = payload.new ?? payload.old
        if (!fila || !idsArea.has(fila.area_id)) return
        setItems(prev => {
  ...
  return { proyectos, areas, items, cargando, error, recargar, todos }
}

// ---------------------------------------------------------------------
// Herrajes
//
// Un bulto de herrajes es un ítem normal con su etiqueta y sus dos escaneos.
// Lo que cambia es que lleva dentro una lista de piezas del catálogo.
//
// Ese contenido no son ítems a propósito: una caja con veinte bisagras y
// cuatro correderas es UN bulto que sube al camión una vez. Si cada herraje
// fuera un ítem habría que escanear veinticuatro etiquetas.
// ---------------------------------------------------------------------

export function useHerrajes() {
  ...
  const recargar = useCallback(async () => {
    const { data, error } = await supabase
      .from('herrajes').select('*')
      .is('deleted_at', null)
      .order('nombre')
    if (error) setError(error.message)
    else { setHerrajes(data ?? []); setError(null) }
    setCargando(false)
  }, [])
  ...
  const crear = useCallback(async (campos) => {
    const { data, error } = await supabase.from('herrajes').insert({
      codigo: campos.codigo.trim().toUpperCase(),
      nombre: campos.nombre.trim(),
      marca: campos.marca?.trim() || null,
      medida: campos.medida?.trim() || null,
    ...
    if (error) {
      throw new Error(
        error.code === '23505'
          ? `Ya existe un herraje con el código ${campos.codigo.toUpperCase()}`
          : traducirError(error)
      )
    }
    await recargar()
  ...
  const actualizar = useCallback(async (id, campos) => {
    const { error } = await supabase.from('herrajes').update(campos).eq('id', id)
    if (error) throw new Error(traducirError(error))
    await recargar()
  }, [recargar])

  const borrar = useCallback(async (id) => {
    const { error } = await supabase.from('herrajes')
      .update({ deleted_at: new Date().toISOString() }).eq('id', id)
    if (error) {
      throw new Error(
        error.code === '23503'
          ? 'Ese herraje está dentro de un bulto ya creado. Quítalo de ahí antes de borrarlo.'
          : traducirError(error)
      )
    }
    await recargar()
  ...
  return { herrajes, cargando, error, recargar, crear, actualizar, borrar }
}

/** Contenido de uno o varios bultos, ya formateado para mostrar. */
export async function contenidoBultos(itemIds) {
  if (!itemIds?.length) return {}
  const { data, error } = await supabase
    .from('bulto_contenido').select('*')
    .in('item_id', itemIds)
    .order('orden')
  if (error) throw new Error(traducirError(error))
  const mapa = {}
  for (const fila of data ?? []) {
    if (!mapa[fila.item_id]) mapa[fila.item_id] = []
    mapa[fila.item_id].push(fila)
  }
  return mapa
}

export async function crearBultoHerrajes(areaId, nombre, contenido, notas) {
  const { data, error } = await supabase.rpc('crear_bulto_herrajes', {
    p_area: areaId,
    p_nombre: nombre?.trim() || 'Herrajes',
    p_contenido: contenido,
    p_notas: notas?.trim() || null
  })
  if (error) throw new Error(traducirError(error, 'crear_bulto_herrajes'))
  return Array.isArray(data) ? data[0] : data
}

export async function actualizarBultoHerrajes(itemId, contenido) {
  const { data, error } = await supabase.rpc('actualizar_bulto_herrajes', {
    p_item: itemId, p_contenido: contenido
  })
  if (error) throw new Error(traducirError(error, 'actualizar_bulto_herrajes'))
  return data
}

/** Contenido de los bultos de herrajes que viajan en un despacho. */
export async function herrajesDelViaje(viajeId) {
  const { data, error } = await supabase
    .from('viaje_herrajes').select('*')
    .eq('viaje_id', viajeId)
    .order('item_codigo').order('orden')
  if (error) throw new Error(traducirError(error))
  const mapa = {}
  for (const fila of data ?? []) {
    if (!mapa[fila.item_id]) mapa[fila.item_id] = []
