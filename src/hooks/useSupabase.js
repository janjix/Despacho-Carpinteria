...
export function useViajesAbiertos() {
  ...
  const activo = viajes.find(v => v.es_activo) ?? null
  return { viajes, activo, recargar }
}

/**
 * Abre un viaje y lo deja como activo del taller.
 *
 * El activo no es una preferencia de pantalla: solo se carga un camión a la
 * vez. Vive en la base para que el viaje que se abre en la laptop sea contra
 * el que escanea el televisor del galpón.
 */
export async function crearViaje(camionId, destino) {
  const { data, error } = await supabase.rpc('crear_viaje', {
    p_camion: camionId, p_destino: destino?.trim() || null
  })
  if (error) throw new Error(error.message)
  return Array.isArray(data) ? data[0] : data
}

/** Cambia el viaje activo para todos los dispositivos a la vez. */
export async function activarViaje(viajeId) {
  const { data, error } = await supabase.rpc('activar_viaje', { p_viaje: viajeId })
  if (error) throw new Error(error.message)
  return data
}
