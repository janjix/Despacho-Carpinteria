...
export function EscaneoProvider({ children, activo = true }) {
  ...
  // El viaje activo no se elige: es el camión que está abierto.
  //
  // Antes se guardaba por dispositivo en el navegador, y eso rompía el flujo
  // real del taller: se abría el viaje en la laptop y el televisor seguía sin
  // saberlo, así que todo lo escaneado desde ahí quedaba sin camión.
  //
  // Al deducirlo del estado de la base, cualquier pantalla escanea contra el
  // mismo camión sin que nadie configure nada.
  const { viajes } = useViajesAbiertos()
  const viaje = viajes[0] ?? null
  const viajeId = viaje?.id ?? null
  const variosAbiertos = viajes.length > 1
  ...
  const valor = {
    viajes, viaje, viajeId, variosAbiertos,
