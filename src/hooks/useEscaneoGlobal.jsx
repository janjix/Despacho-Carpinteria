...
import { anularEscaneo, registrarEscaneo, useViajesAbiertos } from './useSupabase'
import { hoyVenezuela } from '../lib/codigos'
import { despertarAudio, sonar, vibrar } from '../lib/sonido'

const CLAVE_VIAJE = 'tecc.viajeActivo'
...
export function EscaneoProvider({ children, activo = true }) {
  const [viajeId, setViajeIdEstado] = useState(() => {
    try { return localStorage.getItem(CLAVE_VIAJE) || null } catch { return null }
  })
  ...
  const setViajeId = useCallback((id) => {
    setViajeIdEstado(id)
    try {
      if (id) localStorage.setItem(CLAVE_VIAJE, id)
      else localStorage.removeItem(CLAVE_VIAJE)
    } catch { /* modo privado: se pierde al recargar, nada más */ }
  }, [])

  // Si el viaje guardado ya se cerró, se suelta en vez de fallar en silencio
  useEffect(() => {
    if (viajeId && viajes.length && !viajes.some(v => v.id === viajeId)) setViajeId(null)
  }, [viajeId, viajes, setViajeId])

  // Un solo viaje abierto: se elige solo. Con dos o más decide la persona.
  useEffect(() => {
    if (!viajeId && viajes.length === 1) setViajeId(viajes[0].id)
  }, [viajeId, viajes, setViajeId])
  ...
  const procesar = useCallback(async (codigo) => {
    ...
    setOcupado(true)
    try {
      const r = await registrarEscaneo(codigo, 'auto', viajeId)
