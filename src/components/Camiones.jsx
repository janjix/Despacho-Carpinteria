...
import {
  asignarAViaje, borrarViaje, cambiarEstadoViaje, contenidoViaje, herrajesDelViaje,
  crearViaje, useCamiones, useItemsSinViaje, useViajes
} from '../hooks/useSupabase'
import { fechaHora, hora } from '../lib/codigos'
import { guiaDespacho, nombreGuia } from '../lib/guiaDespacho'
...
function DetalleViaje({ viaje, onCerrar, onCambio, onRefrescar }) {
  const [contenido, setContenido] = useState(null)
  const [error, setError] = useState(null)
  const { items: sinViaje, recargar: recargarSinViaje } = useItemsSinViaje()
  const [asignando, setAsignando] = useState(false)
  const admiteCarga = viaje?.estado === 'cargando' || viaje?.estado === 'despachado'
      ...
      <div className="space-y-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {viaje.estado === 'cargando' && (
            <span className="text-[13px] font-display uppercase tracking-wide text-cargado">
              Recibiendo escaneos
            </span>
          )}
        ...
        <div className="flex flex-wrap gap-3">
          {/* Una copia por defecto.
              Antes salían siempre tres ejemplares (taller, transporte y obra)
              y sin explicación parecen páginas repetidas. Quien las necesite
              tiene el botón al lado. */}
          <Boton
            variante="solido"
            disabled={!contenido?.length}
            onClick={async () => {
              // El detalle de los bultos de herrajes va dentro de la guía:
              // en obra se comprueba sin abrir otro papel
              const herrajes = await herrajesDelViaje(viaje.id)
              const doc = await guiaDespacho(viaje, contenido, 1, herrajes)
              doc.save(nombreGuia(viaje))
            }}
          >
          ...
          <Boton
            ...
            onClick={async () => {
              const herrajes = await herrajesDelViaje(viaje.id)
              const doc = await guiaDespacho(viaje, contenido, 3, herrajes)
              doc.save(nombreGuia(viaje).replace('.pdf', ' (3 copias).pdf'))
            }}
            title="Original para el taller, copia para el transporte y copia para la obra"
          >
            3 copias
          </Boton>
          {viaje.estado === 'cargando' && (
        ...
        {admiteCarga && sinViaje.length > 0 && (
          <div className="border border-borde border-l-4 border-l-embalado bg-white px-4 py-3">
            <p className="font-display font-semibold text-[13px] uppercase tracking-wide mb-2">
              {sinViaje.length} bultos cargados sin camión
            </p>
            <p className="text-[13px] text-tenue mb-3">
              Se escanearon por segunda vez cuando no había viaje activo. Están
              marcados como cargados, pero nadie sabe en qué camión subieron.
            </p>
            <ul className="text-[13px] mb-3 max-h-32 overflow-y-auto divide-y divide-borde border border-borde">
              {sinViaje.slice(0, 12).map(i => (
                <li key={i.id} className="flex flex-wrap gap-x-3 px-3 py-1.5">
                  <span className="font-codigo text-[12px] text-tenue">{i.codigo}</span>
                  <span className="font-semibold">{i.nombre}</span>
                  <span className="text-tenue">{i.proyecto}</span>
                  <span className="ml-auto text-[12px] text-tenue">{hora(i.cargado_at)}</span>
                </li>
              ))}
            </ul>
            {sinViaje.length > 12 && (
              <p className="text-[12px] text-tenue mb-3">y {sinViaje.length - 12} más</p>
            )}
            <Boton
              disabled={asignando}
              onClick={async () => {
                setAsignando(true); setError(null)
                try {
                  await asignarAViaje(viaje.id, sinViaje.map(i => i.id))
                  await recargarSinViaje()
                  setContenido(await contenidoViaje(viaje.id))
                  onRefrescar?.()
                } catch (e) { setError(e.message) }
                finally { setAsignando(false) }
              }}
            >
              {asignando ? 'Asignando' : `Asignarlos a ${viaje.codigo}`}
            </Boton>
          </div>
        )}
