...
import {
  activarViaje, asignarAViaje, borrarViaje, cambiarEstadoViaje, contenidoViaje,
    ...
    <Modal abierto titulo={`Viaje ${viaje.codigo}`} onCerrar={onCerrar} ancho="max-w-3xl">
      <div className="space-y-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {viaje.es_activo && (
            <span className="inline-flex items-center px-2 py-1 text-[11px] font-display
                             font-semibold uppercase tracking-wider bg-tinta text-papel">
              Todo lo que se escanee entra aquí
            </span>
          )}
        ...
        <div className="flex flex-wrap gap-3">
          {viaje.estado === 'cargando' && !viaje.es_activo && (
            <Boton onClick={async () => { await activarViaje(viaje.id); onRefrescar?.() }}>
              Cargar en este camión
            </Boton>
          )}
          <Boton
            variante="solido"
            ...
            onClick={async () => {
              const doc = await guiaDespacho(viaje, contenido, 3)
