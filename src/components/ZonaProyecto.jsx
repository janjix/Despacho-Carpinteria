...
export default function ZonaProyecto({
  ...
  // Lo que se guarda es por dónde va la lista, no el número de página.
  //
  // Con un número de página, cualquier cambio en cuántas tarjetas caben
  // cambia también cuántas páginas hay, y la vista salta: estando en la
  // última, si el total baja de 5 a 4 hay que ir a otra parte, y al volver a
  // subir se regresa. Ese vaivén es el parpadeo.
  //
  // Guardando el índice del primer grupo visible, la posición sigue siendo
  // válida aunque el reparto cambie: se muestra desde ahí y ya está.
  ...
  // La capacidad se recuerda: si una medición puntual devuelve algo distinto,
  // no se reparte la lista de nuevo.
  const capacidadFija = useRef(null)
  if (capacidad && capacidad > 0) capacidadFija.current = capacidad
  const porPagina = Math.max(
    1,
    capacidad && capacidad > 0 ? capacidad : (capacidadFija.current || grupos.length || 1)
  )

  const inicio = grupos.length ? Math.min(desde, Math.max(0, grupos.length - 1)) : 0
  const actual = grupos.slice(inicio, inicio + porPagina)
  const totalPag = Math.max(1, Math.ceil(grupos.length / porPagina))
  const pagina = Math.min(totalPag - 1, Math.floor(inicio / porPagina))

  const gruposRef = useRef(grupos)
  useEffect(() => { gruposRef.current = grupos }, [grupos])
  const porPaginaRef = useRef(porPagina)
  useEffect(() => { porPaginaRef.current = porPagina }, [porPagina])
  ...
  // escaneo.
  // El ritmo lo marca el reloj, no el temporizador.
  //
  // Un intervalo que cuenta doce segundos es frágil: si algo lo recrea (una
  // remedición, un cambio de props, un render de más), la cuenta empieza de
  // cero o se solapan dos, y las páginas pasan a trompicones. Aquí se
  // comprueba cada segundo cuánto ha pasado desde el último cambio real. Da
  // igual cuántas veces se recree: el momento del cambio lo decide la hora.
  const ultimoCambio = useRef(Date.now() + desfase)

  useEffect(() => {
    const t = setInterval(() => {
      const total = gruposRef.current.length
      const n = porPaginaRef.current
      if (total <= n) return
      if (Date.now() - ultimoCambio.current < SEGUNDOS_POR_PAGINA * 1000) return
      ultimoCambio.current = Date.now()
      setDesde(d => {
        const siguiente = d + n
        return siguiente >= total ? 0 : siguiente
      })
    }, 1000)
  ...
  useEffect(() => {
    ...
    const n = porPaginaRef.current
    const i = lista.findIndex(g => g.items.some(
      x => x.codigo.toUpperCase().replace(/[^A-Z0-9]/g, '') === resaltado))
    if (i < 0) return
    // El reloj vuelve a cero para que lo que acaba de aparecer no cambie un
    // segundo después.
    ultimoCambio.current = Date.now()
    setDesde(Math.floor(i / n) * n)
  }, [resaltado])
  ...
  // Leyenda de la página: qué área es cada color. Solo las que están a la
  // vista, en el orden en que aparecen.
  const leyenda = []
  for (const g of actual) {
    if (!leyenda.some(x => x.nombre === g.areaNombre)) {
      leyenda.push({ nombre: g.areaNombre, color: g.areaColor })
    }
  }
  const leyendaDe = (tam) => (
    <span className="inline-flex flex-wrap items-center gap-x-3 gap-y-0.5">
      {leyenda.map(a => (
        <span key={a.nombre}
              className="font-display font-semibold uppercase tracking-wide text-white/75 whitespace-nowrap"
              style={{
                fontSize: `${tam * e}rem`,
                borderBottom: `${0.18 * e}rem solid ${a.color}`,
    ...
    <section className="flex flex-col min-h-0 min-w-0 h-full">
      {conCabecera && (
        ...
        >
          ...
          <span className="text-white/45" style={{ fontSize: `${0.85 * e}rem` }}>
            {fechaCorta(proyecto?.fecha_despacho)}
          </span>
          {leyenda.length > 0 && leyendaDe(0.72)}
      ...
      {!conCabecera && leyenda.length > 0 && (
        <div className="shrink-0" style={{ marginBottom: `${0.4 * e}rem` }}>
          {leyendaDe(0.78)}
        </div>
      )}
