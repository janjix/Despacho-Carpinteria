...
export default function ZonaProyecto({
  ...
  // no se reparte la lista de nuevo. Solo cambia cuando el espacio cambia de
  // verdad, y eso ya está filtrado por la histéresis de la medición.
  ...
  const paginas = useMemo(() => {
    ...
    const n = capacidad && capacidad > 0
      ? capacidad
      : capacidadFija.current || grupos.length || 1
    const trozos = []
    for (let i = 0; i < grupos.length; i += n) trozos.push(grupos.slice(i, i + n))
    return trozos.length ? trozos : [[]]
  }, [grupos, capacidad])

  // Si el total de páginas se reduce y la actual ya no existe, se retrocede a
  // la última válida en lugar de saltar al principio: volver a la página 1
  // cada vez que el total baila es justo lo que se ve como parpadeo.
  useEffect(() => {
    setPagina(p => (p < paginas.length ? p : Math.max(0, paginas.length - 1)))
  }, [paginas.length])
  ...
  useEffect(() => {
    ...
    for (let p = 0; p < actuales.length; p++) {
      ...
      if (hay) {
        // El reloj de rotación vuelve a cero para que la página que acaba de
        // aparecer no cambie un segundo después.
        ultimoCambio.current = Date.now()
        setPagina(p)
        return
      }
