...
function Tarjeta({ grupo, e, resaltada, mostrarProyecto }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        height: `${ALTO_TARJETA * e}rem`
      }}
    >
      <div>
        <span
          className="font-display font-bold leading-[1.1] block overflow-hidden"
          style={{
            fontSize: `${1.6 * e}rem`,
            display: '-webkit-box',
            WebkitLineClamp: 3,
        ...
        <span
          ...
          style={{ fontSize: `${0.85 * e}rem`, marginTop: `${0.2 * e}rem` }}
        >
      ...
      {grupo.items.length > 1 && estado !== 'cargado' && (
        <div className="flex" style={{ height: `${0.5 * e}rem`, gap: '2px', marginTop: `${0.5 * e}rem` }}>
          {grupo.items.map(i => (
...
export default function Panel({ proyectoId, onCambiarProyecto, onSalir }) {
  ...
  // Cambia cada vez que hay que reiniciar el temporizador de rotación
  const [reinicio, setReinicio] = useState(0)
  const [resaltado, setResaltado] = useState(null)
  const [reloj, setReloj] = useState(() => new Date())
  const [controles, setControles] = useState(false)
  const [espacio, setEspacio] = useState({ ancho: 0, alto: 0 })
  const zonaRejilla = useRef(null)

  const e = ajustes.escala
  const vertical = Boolean(ajustes.vertical)

  // Cuántas tarjetas caben de verdad.
  //
  // Antes era un número fijo elegido a mano, y por eso la última fila se
  // cortaba: cuántas entran depende del tamaño de letra, de la orientación y
  // de la pantalla concreta, no de lo que uno haya elegido en el menú.
  useEffect(() => {
    const zona = zonaRejilla.current
    if (!zona) return
    const medir = () => setEspacio({ ancho: zona.clientWidth, alto: zona.clientHeight })
    medir()
  ...
  // Escala efectiva: si con el tamaño elegido no entra ni una fila completa
  // (pantalla pequeña o densidad mal configurada en el TV Box), se reduce
  // hasta que quepa. Es preferible letra algo menor a una tarjeta cortada.
  const escalaUsada = useMemo(() => {
    if (!espacio.alto) return e
    const rem = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16
    let usada = e
    while (usada > 0.45 && ALTO_TARJETA * usada * rem > espacio.alto) usada -= 0.05
    return Math.round(usada * 100) / 100
  }, [espacio.alto, e])

  const capacidad = useMemo(() => {
    if (!espacio.ancho || !espacio.alto) return null
    // 1rem son los píxeles del tamaño base del documento
    const rem = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16
    const anchoMin = (vertical ? ANCHO_TARJETA_VERTICAL : ANCHO_TARJETA) * escalaUsada * rem
    const alto = ALTO_TARJETA * escalaUsada * rem
    const hueco = HUECO * escalaUsada * rem
  ...
  }, [espacio, escalaUsada, vertical])

  const porPagina = ajustes.porPagina === 'auto'
    ? (capacidad ?? 20)
    : Math.max(1, Number(ajustes.porPagina) || 20)
  ...
  const grupos = useMemo(() => {
    ...
    // Áreas agrupadas por proyecto, y dentro de cada área los muebles en
    // orden alfabético: buscar un nombre en una pared es más rápido que
    // buscarlo en el orden en que se cargó la lista.
    const ordenAreas = [...areas].sort((a, b) => {
      const pa = nombreProyecto[a.proyecto_id] ?? ''
      const pb = nombreProyecto[b.proyecto_id] ?? ''
      return comparar.compare(pa, pb) || (a.orden ?? 0) - (b.orden ?? 0)
    })

    return ordenAreas.flatMap(a =>
      (porArea.get(a.id) ?? [])
        .sort((x, y) => comparar.compare(x.base.nombre, y.base.nombre))
        .map(g => ({
          ...g,
          areaNombre: a.nombre,
          proyectoNombre: nombreProyecto[a.proyecto_id] ?? ''
        }))
    )
  }, [areas, enTaller, proyectos])
  ...
  // El array de páginas se recrea en cada render. Guardarlo en una ref
  // permite consultarlo desde el efecto del escaneo sin ponerlo como
  // dependencia, que dispararía el efecto en bucle.
  const paginasRef = useRef(paginas)
  useEffect(() => { paginasRef.current = paginas }, [paginas])

  // La rotación se reinicia al saltar a una página: si no, la página que
  // acaba de aparecer podría cambiar un segundo después y nadie vería nada.
  useEffect(() => {
  ...
  // Al escanear, el panel salta a la página donde está ese bulto. En una
  // pantalla que rota sola, ver el cambio es lo que confirma al operario que
  // el escaneo entró: si el mueble está en la página cuatro, no se entera.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
            ...
            <p className="text-white/45 mt-1 truncate" style={{ fontSize: `${0.95 * e}rem` }}>
              {todos
                ...
                : `${proyecto?.cliente ? `${proyecto.cliente} · ` : ''}Despacho ${fechaCorta(proyecto?.fecha_despacho)}` +
                  (areasEnPagina.length ? ` · ${areasEnPagina.join(' · ')}`: '')}
            </p>
          </div>
        ...
        {/* Si se escanea desde este televisor, hay que ver a qué camión va.
            Sin viaje abierto los bultos quedarían cargados sin asignar. */}
        {!viaje && (
          <div
            className="shrink-0 font-display font-bold uppercase tracking-wide"
            style={{
              ...
              marginTop: `${1 * e}rem`,
              padding: `${0.6 * e}rem ${1.2 * e}rem`,
              fontSize: `${1.3 * e}rem`
            }}
          >
            Ningún camión abierto · lo que se cargue quedará sin asignar
          </div>
        )}
            ...
            <div
              ...
              style={{
                ...
                gridTemplateColumns: `repeat(auto-fill, minmax(${
                  (vertical ? ANCHO_TARJETA_VERTICAL : ANCHO_TARJETA) * escalaUsada}rem, 1fr))`,
                gridAutoRows: `${ALTO_TARJETA * escalaUsada}rem`
              }}
            >
              {paginaActual.map(g => (
                <Tarjeta
                  ...
                  grupo={g}
                  e={escalaUsada}
                  mostrarProyecto={todos}
                  resaltada={Boolean(resaltado) && resaltado !== '—' && g.items.some(i =>
                    i.codigo.toUpperCase().replace(/[^A-Z0-9]/g, '') === resaltado)}
                />
              ))}
            </div>
          )}
        ...
        >
          ...
          <div>
            ...
            <p className="text-[12px] text-white/35 mt-2 leading-relaxed">
              {ajustes.porPagina === 'auto'
                ? `Cabe${capacidad === 1 ? '' : 'n'} ${capacidad ?? '…'} en esta pantalla` +
                  (escalaUsada < e ? `, reduciendo la letra para que entren enteras.` : ' con el tamaño actual.')
                : 'Con un número fijo puede que la última fila no entre y se corte.'}
            </p>
          </div>
