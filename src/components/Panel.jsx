  ...
  return (
    <div className="min-h-[100dvh] bg-panel text-white panel-tv overflow-hidden flex flex-col">
      <header className="flex flex-wrap items-center gap-x-8 gap-y-3 px-6 xl:px-10 py-5 border-b border-white/10">
        {/* Salida discreta: el panel vive en un televisor, pero alguien tiene
            que poder volver desde una tablet sin buscar la barra del navegador */}
        <button
          onClick={onSalir}
          className="shrink-0 min-h-[48px] min-w-[48px] px-3 -ml-3 text-white/40 hover:text-white
                     font-display uppercase tracking-[0.14em] text-[12px]"
          title="Volver a los proyectos"
        >
          ← Salir
        </button>
      ...
      <footer className="shrink-0 border-t border-white/10 px-6 xl:px-10 py-3 flex flex-wrap items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          <select
            ...
            className="bg-panelsup text-white border border-white/20 min-h-[44px] px-3 text-[14px]"
          >
            {proyectos.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
          </select>
          <button
            onClick={onSalir}
            className="min-h-[44px] px-4 border border-white/25 text-white/70 hover:text-white
