...
export default function Panel({ proyectoId, onCambiarProyecto, onSalir }) {
  ...
  const { viaje, viajes, setViajeId, ultimo: ultimoLocal } = useEscaneoGlobal()
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Si se escanea desde esta pantalla, hay que saber a qué camión sube
            lo que se cargue. Sin viaje el bulto queda huérfano y la guía de
            despacho sale incompleta, así que se avisa en grande. */}
        {!viaje && viajes.length > 0 && (
          <div
            className="shrink-0 font-display font-bold uppercase tracking-wide"
            style={{
          ...
          >
            Sin viaje activo · lo que se cargue quedará sin camión · elige uno en Ajustes
          </div>
        )}
      ...
      >
        <div className="flex flex-col gap-4 p-5 text-[14px]">
          ...
          <label className="block">
            <span className="font-display uppercase tracking-wide text-[12px] text-white/50 block mb-2">
              Camión que se está cargando
            </span>
            <select
              value={viaje?.id ?? ''}
              onChange={(ev) => setViajeId(ev.target.value || null)}
              className="w-full bg-[#1B1F26] text-white border border-white/20 min-h-[44px] px-3 text-[14px]"
            >
              <option value="">Ninguno</option>
              {viajes.filter(v => v.estado === 'cargando').map(v => (
                <option key={v.id} value={v.id}>
                  {v.camion_codigo} · {v.codigo}{v.destino ? ` → ${v.destino}` : ''}
                </option>
              ))}
            </select>
            <p className="text-[12px] text-white/35 mt-2 leading-relaxed">
              Es el mismo para todos los dispositivos. Cambiarlo aquí lo cambia
              también en la laptop.
            </p>
          </label>

          {/* Viaje activo de ESTE dispositivo. El televisor y la laptop llevan
              el suyo por separado, así que abrir un viaje desde la laptop no
              lo selecciona aquí. */}
          <label className="block">
            <span className="font-display uppercase tracking-wide text-[12px] text-white/50 block mb-2">
              Cargar en el camión
            ...
            {viajes.length ? (
              <>
                <select
                  value={viajeId ?? ''}
                  onChange={(ev) => setViajeId(ev.target.value || null)}
                  className="w-full bg-[#1B1F26] text-white border min-h-[48px] px-3 text-[15px]"
                  style={{ borderColor: viaje ? 'rgba(255,255,255,.2)' : ESTADOS.embalado.hex }}
                >
                ...
                <p className="text-[12px] text-white/35 mt-2 leading-relaxed">
                  Solo hace falta si se escanea desde esta pantalla. Con un único
                  viaje abierto se elige solo.
                </p>
              </>
            ) : (
              <p className="text-[13px] text-white/40 leading-relaxed">
                No hay viajes abiertos. Abre uno desde Camiones antes de empezar
                a cargar.
              </p>
            )}
          </label>
