...
import { COLORES_AREA, ESTADOS, esHoy, fechaCorta, hora, resumir } from '../lib/codigos'
...
function Tarjeta({ grupo, e, resaltada, mostrarProyecto, alto = 7.4 }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: `${0.42 * e}rem ${0.7 * e}rem`,
        height: `${alto * e}rem`
      }}
    >
      {/* El nombre vive en el espacio que sobra, no al revés.
          Antes tenía altura propia y con dos líneas empujaba fuera el pie y
          la tira de bultos, que quedaban recortados por el overflow: la
          información más importante desaparecía sin avisar. Ahora el pie es
          intocable y el nombre se recorta si no cabe. */}
      <div className="flex-1 min-h-0 overflow-hidden">
        <span
          className="font-display font-bold leading-[1.05] block overflow-hidden"
          style={{
            // Los nombres largos bajan de cuerpo para caber en dos líneas
            fontSize: `${(grupo.base.nombre.length > 34 ? 1.15
                        : grupo.base.nombre.length > 22 ? 1.32
                        : 1.55) * e}rem`,
            display: '-webkit-box',
            WebkitLineClamp: 2,
        ...
        {/* El área va debajo y más chica, subrayada con su color.
            El subrayado es un borde inferior y no text-decoration: el
            navegador de los Samsung no entiende el grosor ni el color de
            text-decoration, y un borde se ve igual en cualquier sitio. La
            opacidad va solo en el texto para que la línea salga con el color
            pleno, también sobre fondo amarillo o verde. */}
        <div style={{ marginTop: `${0.1 * e}rem`, lineHeight: 1.3 }}>
          <span
            className="font-display font-semibold uppercase tracking-[0.1em] inline-block max-w-full truncate align-top"
            style={{
              fontSize: `${0.75 * e}rem`,
              borderBottom: `${0.2 * e}rem solid ${grupo.areaColor ?? 'transparent'}`,
              paddingBottom: `${0.04 * e}rem`
            }}
          >
      ...
      {/* Pie: qué falta y cuánto llevamos.
          Con varios bultos el texto dice las dos etapas, no solo los
          cargados: durante la carga lo que importa es cuántos quedan por
          subir de los que ya están embalados. */}
      <div className="shrink-0" style={{ marginTop: `${0.25 * e}rem` }}>
        <div className="flex items-end justify-between gap-2">
          <span
            className="font-display font-semibold uppercase tracking-wider opacity-85 truncate"
            style={{ fontSize: `${0.88 * e}rem` }}
          >
            {!multiple
              ? (estado === 'pendiente' ? 'Pendiente' : estado === 'embalado' ? 'Embalado' : 'Cargado')
              : estado === 'cargado'
                ? `${total} de ${total} cargados`
                : cargados > 0
                  ? `${embalados} embalados · ${cargados} cargados`
                  : `${embalados} de ${total} embalados`}
          </span>
          <span
            className="font-codigo tabular-nums opacity-70 shrink-0"
            style={{ fontSize: `${0.88 * e}rem` }}
          >
            {multiple ? total : grupo.base.codigo.split('-').slice(-1)[0]}
          </span>
        </div>

        {/* Una barrita por bulto, en el color de su estado. Se mantiene
            visible mientras quede algo por cargar: antes desaparecía al
            estar todo embalado, justo cuando empieza a importar. */}
        {multiple && estado !== 'cargado' && (
          total <= 14 ? (
            <div className="flex" style={{ height: `${0.3 * e}rem`, gap: '2px', marginTop: `${0.2 * e}rem` }}>
              {grupo.items.map(i => (
                <span key={i.id} className="flex-1" style={{ background: colorBarrita(i.estado, estado) }} />
              ))}
            </div>
          ) : (
            // Con muchos bultos las barritas quedan de dos píxeles y no se
            // distinguen: se pasa a una barra continua de dos tramos, que dice
            // lo mismo y se ve desde igual de lejos.
            <div className="flex" style={{
              height: `${0.3 * e}rem`, marginTop: `${0.2 * e}rem`,
              background: estado === 'embalado' ? 'rgba(0,0,0,.18)' : 'rgba(255,255,255,.15)'
            }}>
              <span style={{ width: `${(cargados / total) * 100}%`, background: ESTADOS.cargado.hex }} />
              <span style={{
                width: `${((embalados - cargados) / total) * 100}%`,
                background: estado === 'embalado' ? '#8A6200' : ESTADOS.embalado.hex
              }} />
            </div>
          )
        )}
...
export default function Panel({ proyectoId, onCambiarProyecto, onSalir }) {
  ...
  // null = no se pudo comprobar; true / false = Rubik cargó o no
  const [fuenteOk, setFuenteOk] = useState(null)
  ...
  const grupos = useMemo(() => {
    ...
    const ordenadas = [...areas].sort((a, b) =>
      (a.orden ?? 0) - (b.orden ?? 0) || String(a.created_at).localeCompare(String(b.created_at)))

    // Color por posición del área dentro de su proyecto: cada proyecto
    // empieza la paleta de cero, así que en pantalla dividida las primeras
    // áreas de las dos obras comparten color sin confundirse, porque están en
    // columnas distintas.
    const colorDe = {}
    const vistasPorProyecto = {}
    for (const a of ordenadas) {
      const n = vistasPorProyecto[a.proyecto_id] ?? 0
      colorDe[a.id] = COLORES_AREA[n % COLORES_AREA.length]
      vistasPorProyecto[a.proyecto_id] = n + 1
    }

    return ordenadas.flatMap(a =>
      (porArea.get(a.id) ?? [])
        .sort((x, y) => comparar.compare(x.base.nombre, y.base.nombre))
        .map(g => ({
          ...g,
          areaNombre: a.nombre,
          areaColor: colorDe[a.id],
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* La marca va en el pie, que ya existe: no le quita alto a las
              tarjetas, que es lo que se busca en esta pantalla */}
          <img
            src="/fate-blanco.png"
            alt="FATE"
            className="block w-auto opacity-60"
            style={{ height: `${0.9 * e}rem` }}
          />
        </footer>
      </div>
            ...
            <p className="text-[13px] text-white/70 font-codigo leading-relaxed">
              ...
              {medidas.perdido > 60 && (
                <span className="text-embalado">
                  La barra del navegador se lleva {medidas.perdido}px de alto<br />
                </span>
              )}
              {fuenteOk === true && <span>Fuente: Rubik cargada</span>}
              {fuenteOk === false && (
                <span className="text-embalado">
                  Fuente: Rubik no cargó, se está usando la del sistema.
                  Recarga la página; si sigue, falta subir public/fonts.
                </span>
              )}
              {fuenteOk === null && <span className="text-white/40">Fuente: sin comprobar</span>}
            </p>
            {medidas.perdido > 60 && (
