...
import { COLORES_AREA, ESTADOS, ORDEN_ESTADO, abreviar, esHoy, fechaCorta, fechaHora, hora, resumir } from '../lib/codigos'
...
export default function Proyecto({ proyectoId, onVolver }) {
  ...
  // Áreas desplegadas. Plegadas por defecto: en un proyecto con ocho áreas y
  // ciento cincuenta bultos, verlo todo abierto obliga a bajar mucho para
  // encontrar lo que falta. Lo que cada uno abre se recuerda por proyecto.
  const claveAbiertas = `fate.areasAbiertas.${proyectoId}`
  const [abiertas, setAbiertas] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem(claveAbiertas) ?? '[]')) }
    catch { return new Set() }
  })
  const guardarAbiertas = (nuevo) => {
    setAbiertas(nuevo)
    try { localStorage.setItem(claveAbiertas, JSON.stringify([...nuevo])) } catch { /* da igual */ }
  }
  const alternarArea = (id) => {
    const nuevo = new Set(abiertas)
    if (nuevo.has(id)) nuevo.delete(id); else nuevo.add(id)
    guardarAbiertas(nuevo)
  }
  ...
  // Mismo color que en el panel del televisor: quien busca un mueble en la
  // pantalla y luego lo busca aquí, reconoce el área por el color.
  const colorArea = useMemo(() => {
    ...
    return Object.fromEntries(ordenadas.map((a, i) => [a.id, COLORES_AREA[i % COLORES_AREA.length]]))
  }, [areas])
  ...
  const agregarArea = async () => {
    ...
    setCreandoArea(true)
    try {
      const nueva = await crearArea(proyectoId, nombre, areas.length)
      if (nueva?.id) guardarAbiertas(new Set([...abiertas, nueva.id]))
      setNuevaArea('')
      await recargar()
    } catch (e) {
      ...
      {!areas.length ? (
        ...
        <div className="space-y-5">
          <div className="flex justify-end gap-1 -mb-2">
            <button
              onClick={() => guardarAbiertas(new Set(areas.map(a => a.id)))}
              className="min-h-[40px] px-3 text-[12px] font-display uppercase tracking-wide text-tenue hover:text-tinta">
              Desplegar todas
            </button>
            <button
              onClick={() => guardarAbiertas(new Set())}
              className="min-h-[40px] px-3 text-[12px] font-display uppercase tracking-wide text-tenue hover:text-tinta">
              Plegar todas
            </button>
          </div>
          {areas.map(area => {
            ...
            // Falta = todo lo que todavía no ha subido al camión
            const faltan = r.total - r.cargado
              ...
              <section key={area.id}>
                <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                  <button
                    ...
                    className="flex items-center gap-3 flex-wrap text-left min-h-[48px] -ml-1 pl-1 pr-2 hover:bg-white/60"
                  >
                    <span className="text-tenue text-[14px] w-3 shrink-0">{abierta ? '▾' : '▸'}</span>
                    <span className="inline-block w-3.5 h-3.5 rounded-sm shrink-0"
                          style={{ background: colorArea[area.id] }} />
                    <h2 className="text-xl font-bold uppercase tracking-tight">{area.nombre}</h2>
                    {r.total === 0 ? (
                      <span className="text-[13px] text-tenue">Sin ítems</span>
                    ) : faltan === 0 ? (
                      <span className="text-[12px] font-display font-semibold uppercase tracking-wider px-2 py-0.5"
                            style={{ background: ESTADOS.cargado.fondo, color: ESTADOS.cargado.texto }}>
                        Completa · {r.total} cargados
                      </span>
                    ) : (
                      <>
                        <span className="text-[12px] font-display font-semibold uppercase tracking-wider px-2 py-0.5 bg-tinta text-papel">
                          Faltan {faltan}
                        </span>
                        <span className="text-[13px] text-tenue">
                          {r.pendiente > 0 && `${r.pendiente} por embalar`}
                          {r.pendiente > 0 && r.embalado > 0 && ' · '}
                          {r.embalado > 0 && `${r.embalado} por cargar`}
                          {' · '}de {r.total}
                        </span>
                      </>
                    )}
