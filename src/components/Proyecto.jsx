...
import {
  ...
  duplicarArea, editarGrupo, marcarImpresas, renombrarArea, anularEscaneo, useProyecto
...
function FilaGrupo({ grupo, viajes, onImprimir, onEditar, onBorrar, onRevertir }) {
                  ...
                  <ul className="grid gap-2">
                    {grupos.map(g => (
                      <FilaGrupo
                        ...
                        onRevertir={async (it) => {
                          const motivo = prompt(
                            `Motivo para anular el escaneo de ${it.codigo}`,
                            'error de lectura'
                          )
                          if (motivo === null) return
                          await anularEscaneo(it.id, motivo, false)
                          await recargar()
                        }}
                      />
                    ))}
