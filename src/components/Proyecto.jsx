...
import {
  ...
  duplicarArea, editarGrupo, marcarImpresas, renombrarArea, revertirItem, useProyecto
} from '../hooks/useSupabase'
import { abrirEtiquetas } from '../lib/etiquetas'
import { listaCarga, listaEmbalaje } from '../lib/listasPdf'
import { ESTADOS, abreviar, esHoy, fechaCorta, hora, resumir } from '../lib/codigos'
...
function FilaGrupo({ grupo, onImprimir, onEditar, onBorrar, onRevertir }) {
    ...
    <li className="tarjeta flex items-stretch">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3">
          ...
          >
            <span className="block font-display font-bold text-[17px] truncate">{base.nombre}</span>
          ...
          <ul className="border-t border-borde">
            {grupo.items.map(it => (
              <li key={it.id} className="flex items-center gap-3 px-4 py-2 border-b border-borde last:border-0">
                ...
                {it.estado !== 'pendiente' && (
                  <button onClick={() => onRevertir(it)}
                    className="min-h-[40px] px-2 text-[12px] font-display uppercase text-tenue hover:text-alerta">
                    Revertir
                  </button>
                )}
...
export default function Proyecto({ proyectoId, onVolver }) {
  const { proyecto, areas, items, indice, cargando, error, recargar } = useProyecto(proyectoId)
  ...
  const agregarArea = async () => {
    ...
    try {
      await crearArea(proyectoId, nuevaArea, areas.length)
      setNuevaArea('')
      await recargar()
  ...
  const guardarItems = (area) => async (filas) => {
    await crearItems(area, filas, proyecto.codigo_corto)
    await recargar()
  }
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
      ...
      <header className="mt-3 mb-7">
        ...
        <div className="flex flex-wrap gap-3 mt-5">
          ...
          <Boton onClick={async () => {
            const doc = await listaCarga(proyecto, indice.areas, items)
      ...
      <div className="flex flex-wrap gap-2 items-end mb-6 pb-6 border-b border-borde">
        ...
        />
        <Boton onClick={agregarArea} className="mb-6">Agregar área</Boton>
      </div>
                  ...
                  <ul className="grid gap-2">
                    {grupos.map(g => (
                      <FilaGrupo
                        ...
                        onRevertir={async (it) => { await revertirItem(it.id); await recargar() }}
                      />
                    ))}
                  </ul>
                )}
