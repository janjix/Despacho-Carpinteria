...
export default function Escaneo({ onIrACamiones }) {
  const {
    ultimo, historial, contador, viaje, variosAbiertos, procesar, anular
  } = useEscaneoGlobal()
  ...
  return (
    ...
    >
      ...
      {/* Camión que está recibiendo la carga.
          No se elige: en el taller se carga uno a la vez, así que el activo es
          el viaje que esté abierto. Antes había un selector por dispositivo y
          eso hacía que lo escaneado desde el televisor quedara sin camión. */}
        ...
        {!viaje ? (
          <div className="flex flex-wrap items-center gap-3">
            <p className="text-[14px] text-[#5C4300] flex-1 min-w-[220px]">
              Ningún camión abierto. Lo que se embale se registra igual, pero lo
              que se cargue va a quedar sin camión asignado.
            </p>
          ...
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
            ...
            {viaje.destino && (
              <span className="text-[14px] text-tenue">→ {viaje.destino}</span>
            )}
            <span className="text-[13px] text-tenue">
              {[viaje.placa, viaje.conductor].filter(Boolean).join(' · ')}
            </span>
            {variosAbiertos && (
              <span className="text-[12.5px] text-alerta w-full">
                Hay más de un viaje abierto y se está usando el más reciente.
